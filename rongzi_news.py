...
def get_36kr_news():
    res = requests.get("https://www.36kr.com/newsflashes")
    soup = BeautifulSoup(res.content, 'html.parser')
    contents = []
    news_list = []
    for item in soup.select(".item-desc span"):
        text = item.get_text()
...
def compile_page(tag_dict, news_list, deep_news, date_title):
    page = ""
    page += "{} 真格日报 【xxx期】\n\n".format(date_title)
    page += "**********热门赛道**********\n\n"
...
if __name__ == '__main__':
    ...
    date_title = datetime.today().strftime("%Y/%m/%d")
    # print(date)
    ...
    page = compile_page(tag_dict, news_list, deep_news, date_title)
