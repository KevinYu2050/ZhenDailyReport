...
def get_36kr_news():
    ...
    response = requests.post(url, headers={"content-type": "application/json"}, data=json.dumps({"partner_id": "web", "param": {"pageSize": 50, "pageEvent": 1,
                                                                                                                                "pageCallback": "eyJmaXJzdElkIjoxMjE4MjI0NjkzMzM0NDA1LCJsYXN0SWQiOjEyMTgwNDQzMzE0NjMwNDQsImZpcnN0Q3JlYXRlVGltZSI6MTYyMDYyNjUzNDUwNSwibGFzdENyZWF0ZVRpbWUiOjE2MjA2MTU1MjYwOTB9", "siteId": 1, "platformId": 2}}))
    ...
    for item in items:
        ...
        if content.find("融资") == -1:
            news_list.append(content.replace(
                "36氪获悉，", "").replace("截至发稿，", ""))
...
def get_36kr_deep_news():
    ...
    response = requests.post(url, headers={"content-type": "application/json"}, data=json.dumps({"partner_id": "web", "param": {"itemId": "327690223617", "pageSize": 1, "pageEvent": 1,
                                                                                                                                "pageCallback": "eyJmaXJzdElkIjo5MTEyOTgsImxhc3RJZCI6OTAxMTU2LCJmaXJzdENyZWF0ZVRpbWUiOjE2MDIzMjUyNjk3ODIsImxhc3RDcmVhdGVUaW1lIjoxNTk5Mjc3NjA2MTM2fQ", "siteId": 1, "platformId": 2}}))
    items = json.loads(response.text)["data"]["itemList"]
    news_list = []

    for item in items:
        news_list.append(
            (item["templateMaterial"]["content"], item["templateMaterial"]["itemId"]))
